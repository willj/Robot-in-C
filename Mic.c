...
/*********************************************************************************
 *	Initialize the mic on Analog In 6 (PD1), using sequencer 3.
 *
 ********************************************************************************/
...
void initMic (void) {
	...
	// Port D
	// PD1 as Analog In 6
	GPIO_PORTD_DIR_R &= ~0x02;			// Make PD1 input
	GPIO_PORTD_AFSEL_R |= 0x02;			// Enable the alt function for PD1
	GPIO_PORTD_DEN_R &= ~0x02;			// Disable digital on PD1
	GPIO_PORTD_AMSEL_R |= 0x02;			// Enable analog function on PD1
	...
	ADC0_SSMUX3_R += 6;							// Set the channel to AIN6 (PD1)
